<script lang="ts">
  ...
  let { policies, isLoading, onedit }: Props = $props();
  ...
  async function handleDelete(policy: SLAPolicy) {
    ...
    const result = await slaStore.deletePolicy(policy.id);
  ...
  {:else if policies.length === 0}
    <Card.Root>
      <Card.Content class="p-12 text-center">
        ...
        <h3 class="text-lg font-semibold mb-2">No SLA policies yet</h3>
              ...
              <div class="grid grid-cols-3 gap-4 text-sm">
                <div class="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p class="font-medium">{formatTime(policy.firstResponseTime)}</p>
                  </div>
                </div>
                <div class="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p class="font-medium">{formatTime(policy.nextResponseTime)}</p>
                  </div>
                </div>
                <div class="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p class="font-medium">{formatTime(policy.resolutionTime)}</p>
                  </div>
                </div>
              </div>
            </div>
