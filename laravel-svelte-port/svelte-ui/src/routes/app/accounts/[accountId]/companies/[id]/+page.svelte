<script lang="ts">
  ...
  import { ArrowLeft, Globe, Building, Calendar, Pencil, Trash } from 'lucide-svelte';
  ...
  async function loadData() {
    ...
    try {
      ...
      const contactsData = await getContacts(accountId, { company_id: companyId });
      contacts = contactsData.data.payload;
    } catch (error) {
...
<div class="h-full flex flex-col bg-background">
    ...
    <div class="flex items-center justify-between px-6 py-4 border-b">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-xl font-medium">{company?.name || 'Company Details'}</h1>
                 {#if company?.website}
                    <a 
                    ...
                    <Card.Root>
                        ...
                        <Card.Content class="space-y-4">
                             ...
                             <div class="grid grid-cols-2 gap-4">
                                ...
                                {#if company.size}
                                    <div>
                                        <div class="text-sm font-medium text-muted-foreground mb-1">Size</div>
                                        ...
                                        <div class="flex items-center justify-between p-3 border rounded-lg hover:bg-muted/50 transition-colors">
                                            <div class="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <div class="text-sm text-muted-foreground">{contact.email || contact.phoneNumber || 'No contact info'}</div>
                                                </div>
                                            </div>
                                            <Button variant="ghost" size="sm" href={`/app/accounts/${accountId}/contacts/${contact.id}`}>
                                                View
                                            </Button>
                                        </div>
                                    {/each}
