<script lang="ts">
  ...
  let isSubmitting = $derived(portalArticlesStore.uiFlags.isUpdating);
  let isLoading = $derived(portalArticlesStore.uiFlags.isFetchingItem);
  ...
  onMount(async () => {
    ...
    // Load article if needed
    await portalArticlesStore.fetchArticle(portalSlugParam, articleId);
    const article = portalArticlesStore.allArticles.find(
      a => a.id === articleId
    );
  ...
  async function handleSubmit() {
    ...
    const result = await portalArticlesStore.updateArticle(
      portalSlugParam,
      articleId,
      data
    );
...
<SectionLayout title="Edit Article" description="Update help article">
