<script lang="ts">
   ...
   * Browse and manage active contacts
   */
...
<div class="h-full flex flex-col">
  ...
  <div class="p-6 border-b">
    <div class="flex items-center justify-between mb-4">
      <div>
        <h1 class="text-3xl font-bold">Active Contacts</h1>
        <p class="text-muted-foreground">
          Manage your active contact list ({contacts.length} contacts)
        </p>
      </div>
      <Button class="gap-2">
        <Plus class="h-4 w-4" />
    ...
    {:else if filteredContacts().length === 0}
      ...
      <div class="flex items-center justify-center h-full">
        <div class="text-center">
          {#if searchQuery}
            <p class="text-lg mb-2">No active contacts found</p>
            <p class="text-sm text-muted-foreground">
              Try adjusting your search query
            </p>
          {:else}
            <p class="text-lg mb-2">No active contacts yet</p>
