<script lang="ts">
  ...
  // Generally $page.params.label is the raw param value (decoded),
  // but if it contains spaces it might be helpful to double check.
...
<ContactList
  title={`Label: ${label}`}
  {accountId}
  initialFetchParams={{ labels: [label] }}
/>
